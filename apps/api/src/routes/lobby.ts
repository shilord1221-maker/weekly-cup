  ...
  app.get('/api/lobby/:matchId', async (req, reply) => {
    ...
    const lobby = await prisma.lobby.findUnique({
      ...
      include: {
        ...
        teams: { include: { members: { include: { user: { select: { id: true, username: true, avatarUrl: true, activeFrameEffect: true, staticId: { select: { value: true } } } } } } }, orderBy: { slot: 'asc' } },
      },
    });
    ...
    const unassignedMembers = await prisma.lobbyMember.findMany({
      ...
      include: { user: { select: { id: true, username: true, avatarUrl: true, activeFrameEffect: true, staticId: { select: { value: true } } } } },
    });
